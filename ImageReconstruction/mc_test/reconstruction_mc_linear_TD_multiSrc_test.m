...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% add paths
path_mcxlab = '/home/jingjing/Documents//SoftwarePackages/mcx2023/';
...
view(3)
%%
cw_1 = zeros(numsrc*numdet,1);
cw_2 = cw_1;
hist_1_all = zeros(numsrc*numdet, numgate);
hist_2_all = hist_1_all;
for isrc = 1: numsrc
% isrc = 1
...
hist_1_all(numdet*(isrc-1)+1: numdet*isrc,:)=hist_1;
hist_2_all(numdet*(isrc-1)+1: numdet*isrc,:)=hist_2;
...
phi_homo = reshape(log10(hist_1_all(:,idx_tg_select))', numdet* numsrc*ng , 1);
phi_hetero = reshape(log10(hist_2_all(:,idx_tg_select))', numdet*numsrc*ng, 1);
...
tic
jac_all = zeros(vx, vy, vz, numgate,numdet*numsrc);
for isrc = 1:numsrc
...
% zero gates removed
J = reshape(jac_all(:,:,:,idx_tg_select,:), numel(cfg.vol), numdet*numsrc*ng)' ;  % [Nmeas x Nvox]
  
% remove nan
nan_idx = isnan(data_diff);
data_diff_clean = data_diff(~nan_idx);
J_clean = J(~nan_idx, :); % Make sure to remove corresponding rows from J!

data_diff = data_diff_clean;
J = J_clean;
% size(J)
[nx, ny] = size(J);
%% image reconstruction
lambda = 1e3;  % Regularization parameter
...
for iz = 1:size(cfg.vol,3)
    subplot(121)
    imagesc(squeeze( mu_a_recon_3D(:,:,iz)) )
    title(['z ' num2str(iz)])
    colorbar
    % clim([0.005 nanmax(mu_a_recon_3D(:))])
    subplot(122)
    imagesc(squeeze(cfg.vol(:,:, iz)))
