...
%% optical properties at the selected wavelengths
% nirot.wavelengths = [690 725 802 839]; % [nm]
% nirot.iwav = 2;
...
muas_inc = 0.030;
mus_r_inc = 0.9;
%% combine repetitions for a longer exposure 
% - flags: isCombRep
isCombRep = 1; % 1: combine measured data
isPM = 1; % 1: use powermeter
scalerPM = 4e4;
if isCombRep
combineRepetitions(fldr_homo,nirot.wavelengths, group_list,...
    rep_list, src_list, Prefix, isPM,scalerPM)
end
%% STAGE 1 Preparation 
...
if isGenerateMesh
    ...
    fn_mesh = 'exampleVolumeFEM/Cylinder_D90/Cylinder_1_mesh';
    nirot.Volume.Path = [fn_mesh  '.mat'];
    fn_mesh=load(nirot.Volume.Path);
    mesh=fn_mesh.mesh;
end
nirot.det = mesh.meas;
